...
| <<plugins-{type}s-{plugin}-include_partial_data>> |<<boolean,boolean>>|No
...
[id="plugins-{type}s-{plugin}-include_partial_data"]
===== `include_partial_data`

  * Value type is <<boolean,boolean>>
  * Default value is `false`

When enabled, data collected successfully from `walk` or `table` operations that partially fail is preserved in the event.
By default, only data from fully successful operations is included.

When a `walk` or `table` traversal encounters an error mid-way through, the SNMP agent may have already returned
some valid data before the failure. With `include_partial_data => true`, this data is included in the event.
